...
def service():
    ...
    for forecast in forecast_list:
        if forecast[pyiceclient.ICE_FORECASTS_CONCEPT] == "RECOMMENDED" or forecast[
                pyiceclient.ICE_FORECASTS_CONCEPT] == "FUTURE_RECOMMENDED":
            ice_card = card('ICE Recommendation Card', 'info', source)
            ice_card['detail'] = '<table border="1"><tr><td>foo</td><td>bar</td></tr><tr><td>baz</td><td>bat</td></tr></table>' + forecast[pyiceclient.ICE_FORECASTS_GROUP] + ": " + forecast[
                pyiceclient.ICE_FORECASTS_CONCEPT]
            ice_card['links'].append(link('ICE Default Immunization Schedule',
